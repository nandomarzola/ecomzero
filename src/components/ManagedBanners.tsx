...
function BannerImage({ banner, priority = false }: { banner: StoreBanner; priority?: boolean }) {
  ...
  return banner.linkUrl ? <Link href={banner.linkUrl} className="absolute inset-0 focus-visible:outline-2 focus-visible:outline-offset-[-3px] focus-visible:outline-[#A9EC17]">{image}</Link> : image;
}
