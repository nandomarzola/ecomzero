...
UI em dois lugares (ambas com **auto-cálculo** quando há CEP conhecido, desde 2026-07-15):

- **Página de produto** — `ShippingCalculator.tsx` (dentro de `ProductPurchase`, recebe `variantId` + `quantity`; foi removida num rework e RECRIADA no mesmo dia com auto-cálculo). `POST /api/shipping/quote` com `{ variantId, cep, quantidade }`. Auto-cálculo dispara na montagem (se há CEP salvo) e ao trocar variante/quantidade, com debounce 450ms — NUNCA enquanto digita; CEP digitado manualmente usa o botão "Calcular" e, depois do primeiro cálculo, também passa a auto-recalcular. Implementação: efeito com deps `[runQuote, savedCep]` + ref-espelho do CEP (escrita de ref só dentro de efeito — regra `react-hooks/refs` do lint proíbe no render).
- **Carrinho** — `CartCheckoutPanel.tsx` → `POST /api/cart/shipping-quote`. Auto-cotação ao abrir com CEP válido e quando subtotal/productCount mudam (item adicionado/removido → seleção antiga já é invalidada pelo mismatch de `cartSubtotal`), debounce 600ms agrupa mudanças rápidas; nunca roda por cima de seleção válida. Padrão "latest ref" pro callback (ref atualizada em efeito sem deps). Loading discreto: lista esmaecida + spinner (auto) vs botão "Calculando..." (manual).

**CEP do visitante (header/modal)** — `src/lib/client/cepStorage.ts` (localStorage `ecomzero:cep-usuario` + flag `ecomzero:cep-modal-dispensado`, padrão subscribe/snapshot p/ `useSyncExternalStore`, igual `checkoutShippingStorage`). `HeaderCepButton.tsx` (dropdown salvar/trocar CEP — no cluster de ações do `HeaderActions` no desktop `xl+`, substituindo o antigo placeholder "em breve", e numa linha própria no header mobile; estilo alinhado ao padrão "Minha conta": ícone h-5 + principal `text-[11px] font-semibold` + secundário `text-[9px] text-white/45`) e `CepCaptureModal.tsx` (na home, só `status === "unauthenticated"` + sem CEP + sem flag de dispensa; dispensar grava a flag e nunca reaparece). O CEP salvo pré-preenche e ARMA o auto-cálculo nas duas UIs acima. Cliente logado usa `Address` da conta no checkout; esse CEP solto é só conveniência de visitante.
