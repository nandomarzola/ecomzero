...
UI: a calculadora de frete da PÁGINA DE PRODUTO (`ShippingCalculator.tsx`) foi **removida no rework de layout de 2026-07-15** (decisão do dono — o box de compra foi simplificado; a rota `/api/shipping/quote` continua viva e aceita `{ variantId, cep, quantidade }`). O único fluxo de cotação com UI hoje é o do carrinho: `CartCheckoutPanel.tsx` → `POST /api/cart/shipping-quote`.

**CEP do visitante (header/modal)** — `src/lib/client/cepStorage.ts` (localStorage `ecomzero:cep-usuario` + flag `ecomzero:cep-modal-dispensado`, padrão subscribe/snapshot p/ `useSyncExternalStore`, igual `checkoutShippingStorage`). `HeaderCepButton.tsx` (dropdown salvar/trocar CEP, no Header desktop e mobile) e `CepCaptureModal.tsx` (na home, só `status === "unauthenticated"` + sem CEP + sem flag de dispensa; dispensar grava a flag e nunca reaparece). O CEP salvo pré-preenche o campo do `CartCheckoutPanel` (fallback: draft digitado → seleção salva → CEP do visitante) **sem disparar cálculo automático** — cálculo só no clique (evita queimar rate limit/API em toda visita). Cliente logado usa `Address` da conta no checkout; esse CEP solto é só conveniência de visitante.
