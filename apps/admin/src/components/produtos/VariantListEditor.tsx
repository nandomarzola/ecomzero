      ...
      {value.map((row, index) => (
        ...
        >
          <div className="mb-2 flex items-center justify-between">
            <span className="text-xs font-semibold text-white/70">{mode === "simple" ? "Preço, identificação e pacote" : `Variação ${index + 1}`}</span>
            {mode === "variants" && value.length > 2 ? (
              <button
                type="button"
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {mode === "variants" ? (
              <label className="col-span-2 flex flex-col gap-1 text-[11px] text-white/50 sm:col-span-1">
                Nome da variação
                <input className={inputClass} value={row.label} onChange={(e) => update(index, { label: e.target.value })} placeholder="Ex.: 500 ml, Azul, Kit 3 unidades" />
              </label>
            ) : null}
