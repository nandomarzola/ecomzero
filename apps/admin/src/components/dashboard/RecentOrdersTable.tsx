    ...
    <section className="rounded-xl border border-white/[0.08] bg-[#111111]">
      ...
      {orders.length === 0 ? (
        <div className="flex min-h-64 flex-col items-center justify-center px-6 py-10 text-center">
          ...
          <p className="mt-1 max-w-xs text-xs leading-relaxed text-white/35">Os pedidos finalizados pela loja aparecerão aqui automaticamente.</p>
        </div>
      ) : (
        <div className="divide-y divide-white/[0.06]">
          {orders.map((order) => (
            <Link
              ...
              className="grid grid-cols-[1fr_auto_auto] items-center gap-3 px-5 py-3 transition hover:bg-white/[0.025]"
            >
              <div className="min-w-0">
                <p className="truncate text-sm font-medium text-white">{order.customer}</p>
                <p className="truncate text-[11px] text-white/35">#{order.id.slice(0, 8)} · {new Date(order.createdAt).toLocaleString("pt-BR")}</p>
              </div>
              <div className="text-right">
                <p className="text-sm font-semibold text-white">{money(order.total)}</p>
                <p className="mt-0.5 text-[10px] text-[#A9EC17]">
                  {order.shipmentStatus ? shipmentStatusLabel[order.shipmentStatus] ?? order.shipmentStatus : orderStatusLabel[order.status]}
                </p>
              </div>
